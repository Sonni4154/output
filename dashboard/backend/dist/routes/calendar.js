...
router.get('/events', async (req, res) => {
    try {
        ...
        if (start_date && end_date) {
            query = query.where(and(gte(calendarEvents.start_time, new Date(start_date)), lte(calendarEvents.start_time, new Date(end_date))));
        }
        if (calendar_id) {
            query = query.where(eq(calendarEvents.google_calendar_id, calendar_id));
        }
...
router.get('/events/today', async (req, res) => {
    try {
        ...
        const events = await db.query.calendarEvents.findMany({
            where: and(gte(calendarEvents.start_time, today), lte(calendarEvents.start_time, tomorrow), eq(calendarEvents.cancelled, false)),
            with: {
                assignments: {
                    with: {
                        employee: true,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (sequence_order !== undefined)
            updateData.sequence_order = parseInt(sequence_order);
        if (status)
            updateData.status = status;
        if (admin_notes)
            updateData.admin_notes = admin_notes;
        if (employee_notes)
            updateData.employee_notes = employee_notes;
...
router.get('/employee/:employeeId', async (req, res) => {
    try {
        ...
        let whereConditions = [eq(workAssignments.employee_id, parseInt(employeeId))];
...
router.get('/today', async (req, res) => {
    try {
        ...
        tomorrow.setDate(tomorrow.getDate() + 1);
        const assignments = await db.query.workAssignments.findMany({
            where: and(gte(workAssignments.assigned_at, today), lte(workAssignments.assigned_at, tomorrow)),
            with: {
                calendarEvent: true,
                employee: true,
                assignedByEmployee: {
...
router.get('/employees', async (req, res) => {
    try {
        ...
        if (active_only === 'true') {
            query = query.where(eq(employees.active, true));
        }
 ...
 */
router.get('/employees/working-today', async (req, res) => {
    try {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tomorrow = new Date(today);
...
router.get('/notes', async (req, res) => {
    try {
        ...
        let whereConditions = [];
        if (entity_type)
            whereConditions.push(eq(internalNotes.entity_type, entity_type));
        if (entity_id)
            whereConditions.push(eq(internalNotes.entity_id, entity_id));
        if (category)
            whereConditions.push(eq(internalNotes.category, category));
        if (pinned_only === 'true')
            whereConditions.push(eq(internalNotes.pinned, true));
...
router.put('/notes/:id', async (req, res) => {
    try {
        ...
        if (visible_to_all !== undefined)
            updateData.visible_to_all = visible_to_all;
        const [updated] = await db
...
router.post('/clock/in', async (req, res) => {
    try {
        ...
        const [existingEntry] = await db
            .select()
            .from(timeEntries)
            .where(and(eq(timeEntries.employee_id, parseInt(employee_id)), eq(timeEntries.clock_out, null)))
            .limit(1);
        if (existingEntry) {
            return res.status(400).json({
                success: false,
...
router.get('/clock/entries', async (req, res) => {
    try {
        ...
        let whereConditions = [];
        if (employee_id) {
            whereConditions.push(eq(timeEntries.employee_id, parseInt(employee_id)));
        }
        if (start_date) {
            whereConditions.push(gte(timeEntries.clock_in, new Date(start_date)));
        }
        if (end_date) {
            whereConditions.push(lte(timeEntries.clock_in, new Date(end_date)));
        }
