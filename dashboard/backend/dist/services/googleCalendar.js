...
function parseEventDescription(description) {
    ...
    if (customerMatch)
        result.customer_name = customerMatch[1].trim();
    const phoneMatch = description.match(/Phone:\s*(.+?)(?:\n|$)/i);
    if (phoneMatch)
        result.customer_phone = phoneMatch[1].trim();
    const emailMatch = description.match(/Email:\s*(.+?)(?:\n|$)/i);
    if (emailMatch)
        result.customer_email = emailMatch[1].trim();
 ...
 */
function determineServiceType(calendarId, title) {
    const calendarMap = {
 ...
 */
export async function syncCalendarEvents(calendarId) {
    try {
        ...
        const response = await axios.get(`${GOOGLE_CALENDAR_API}/calendars/${encodeURIComponent(calendarId)}/events`, {
            params: {
 ...
 */
export async function appendAssignmentToGoogleEvent(googleEventId, googleCalendarId, employeeName, assignedByName) {
    try {
        const accessToken = await getGoogleAccessToken();
        // Get the current event
        const eventResponse = await axios.get(`${GOOGLE_CALENDAR_API}/calendars/${encodeURIComponent(googleCalendarId)}/events/${googleEventId}`, {
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
        });
        ...
        // Update the event
        await axios.patch(`${GOOGLE_CALENDAR_API}/calendars/${encodeURIComponent(googleCalendarId)}/events/${googleEventId}`, {
 ...
 */
export async function createGoogleCalendarEvent(calendarId, eventData) {
    try {
        const accessToken = await getGoogleAccessToken();
        const response = await axios.post(`${GOOGLE_CALENDAR_API}/calendars/${encodeURIComponent(calendarId)}/events`, {
 ...
 */
export async function updateGoogleCalendarEvent(calendarId, eventId, updates) {
    try {
        const accessToken = await getGoogleAccessToken();
        const updateData = {};
        if (updates.summary)
        ...
        await axios.patch(`${GOOGLE_CALENDAR_API}/calendars/${encodeURIComponent(calendarId)}/events/${eventId}`, updateData, {
            headers: {
                Authorization: `Bearer ${accessToken}`,
                'Content-Type': 'application/json',
            },
...
export async function deleteGoogleCalendarEvent(calendarId, eventId) {
    try {
        const accessToken = await getGoogleAccessToken();
        await axios.delete(`${GOOGLE_CALENDAR_API}/calendars/${encodeURIComponent(calendarId)}/events/${eventId}`, {
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
        });
